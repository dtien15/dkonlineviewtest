document.addEventListener("DOMContentLoaded", () => {
  ...
  function clearPage2Inputs() {
    // Xóa giá trị của các input có data-vnkeys
    document
      .querySelectorAll("#page2 input[data-vnkeys]")
      .forEach((i) => (i.value = ""));

    document
      .querySelectorAll('#pageDichVu input[type="radio"]')
      .forEach((radio) => (radio.checked = false));

    // Xóa dữ liệu của tất cả các select (Choices.js) trong page2
    document.querySelectorAll("#page2 select").forEach((selectEl) => {
      const choicesInstance = selectEl.choices;
      if (choicesInstance) {
        choicesInstance.removeActiveItems(); // Xóa item đang chọn
        choicesInstance.setChoiceByValue(""); // Reset về mặc định nếu có
      } else {
        // fallback nếu chưa có instance (hiếm gặp)
  ...
  // --- Update tiêu đề ---
  function updateHeadings(newText) {
    const headings = document.querySelectorAll(".text-heading");
    headings.forEach((h) => (h.innerText = newText));
  }
  function updateHeadingByFlow() {
    if (currentFlow === "bh") {
      updateHeadings("Đăng ký khám bảo hiểm");
    } else if (currentFlow === "dv") {
      updateHeadings("Đăng ký khám Dịch vụ");
    } else {
      updateHeadings("Đăng ký khám bệnh"); // mặc định
    }
  }

  // --- Nút chuyển trang khám bao hiểm ---
  // 1. Khai báo biến lưu trạng thái luồng
  let currentFlow = "default"; // mặc định, chưa chọn gì

  // 2. Khi nhấn nút "Đăng ký khám Bảo hiểm"
  document.getElementById("registerBtn").addEventListener("click", () => {
    currentFlow = "bh"; // lưu trạng thái là bảo hiểm
  ...
  // 3. Khi nhấn nút "Đăng ký khám Dịch vụ"
  document.getElementById("dkKhamDVBtn").addEventListener("click", () => {
    currentFlow = "dv"; // lưu trạng thái là dịch vụ
    page1.classList.add("d-none");
    pageQR.classList.remove("d-none");
    clearPage2Inputs();
    updateHeadings("Đăng ký khám Dịch vụ");
  ...
  // 4. Chuyển từ QR => Thông tin người bệnh
  ...
  // 7. Quay về trang chính
  document.getElementById("veTrangChinhBtn").addEventListener("click", () => {
    pageXacNhan.classList.add("d-none");
    page1.classList.remove("d-none");
    clearPage2Inputs();

    // Luôn đặt lại tiêu đề mặc định khi về trang chính
    updateHeadings("Đăng ký khám bệnh");
  });

  document.getElementById("quayLaiBtn").addEventListener("click", () => {
    pageQR.classList.add("d-none");
    page1.classList.remove("d-none");
    clearPage2Inputs(); // nếu cần reset dữ liệu

    // Luôn cập nhật lại tiêu đề mặc định
    updateHeadings("Đăng ký khám bệnh");
  });
  ...
  const heading = document.querySelector(".text-wrap1 > h2.text-heading");
  const des = document.querySelector(".text-wrap1 > div.des");
  ...
  tinhEl.choices = tinhChoices;
  quanEl.choices = quanChoices;
  xaEl.choices = xaChoices;
  // Load data from JSON
  ...
  genderEl.choices = genderChoices;
  ...
  const quocTichChoices = new Choices(quocTichEl, {
    searchEnabled: true,
    placeholderValue: "Quốc tịch",
    searchPlaceholderValue: "--- Gõ để tìm ---",
    shouldCloseOnSelect: false,
    position: "top",
  });

  quocTichEl.choices = quocTichChoices;
  ...
  danTocEl.choices = danTocChoices;
  ...
  ngheNghiepEl.choices = ngheNghiepChoices;
